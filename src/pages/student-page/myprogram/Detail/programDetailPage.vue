...
import ProgramDetailTab from './ProgramDetailTab/programDetailTab.vue'
...
// แท็บที่รองรับ
const VALID_TABS = ['program', 'checkInOut'] as const
...
const syncTabFromRoute = () => {
  ...
  const normalized = VALID_TABS.includes(fromParam as typeof VALID_TABS[number]) ? fromParam : 'program'
...
const breadcrumbPath = computed(() => {
  ...
  return router.resolve({
    name: 'MyProgramDetail',
    params: { id, tab: tab.value || 'program' },
  }).href
})

const breadcrumbs = computed(() => ({
  previousPage: { title: 'ประวัติของฉัน', path: '/Student/RecordPage' },
